...
const gulp = require('gulp'),
    ...
    appName = yargs.argv.app || 'main',
    srcAppScriptFile = 'app/scripts/' + appName + 'App.js',
    srcAppCssFile = 'app/styles/main.css',

    distFolders = ['dist/compile/' + appName + '/**', 'dist/rev/' + appName + '/**', 'dist/min/' + appName + '/**'],
    distScriptFile = 'app.js',
    distScriptFolder = 'dist/compile/' + appName + '/scripts',
    distCssFolder = 'dist/compile/' + appName + '/styles',
...
gulp.task('html', function () {
    return gulp.src('app/' + appName + '.html')
        ...
        .pipe(gulp.dest('dist/compile/' + appName));
});
...
gulp.task('fonts', function () {
    return gulp.src(['node_modules/bootstrap/dist/fonts/*.{eot,svg,ttf,woff,woff2}'])
        .pipe(gulp.dest('dist/compile/' + appName + '/fonts'));
});
...
gulp.task('images', function () {
    return gulp.src(['app/images/**'])
        .pipe(gulp.dest('dist/compile/' + appName + '/images'));
});
...
gulp.task('dist', ['env-prod', 'clean', 'appScript', 'appStyle', 'html', 'fonts', 'images', 'extras'], function () {
    return gulp.src('dist/compile/' + appName + '/scripts/*.js')
...
gulp.task('revision', ['dist'], function () {
    return gulp.src(['dist/compile/' + appName + '/**/*.css', 'dist/min/' + appName + '/**/*.js'])
        .pipe($.rev())
...
gulp.task('moveExtrasToRev', ['dist'], function () {
    return gulp.src(['dist/compile/' + appName + '/**/!(*.css|*.js)'])
        .pipe(gulp.dest('dist/rev/' + appName))
});
...
gulp.task('revreplace', ['revision'], function () {
    const manifest = gulp.src('dist/compile/' + appName + '/rev-manifest.json');

    return gulp.src('dist/compile/' + appName + '/index.html')
...
function createWatchBundler() {
    ...
    bundler.plugin(watchify, {poll: 750});
